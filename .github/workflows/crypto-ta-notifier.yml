...
Features:
- Binance API 自動輪替：api.binance.com → api1 → api2 → api3（可由 BINANCE_API_BASE 覆寫起頭）
- 後備資料源：Binance 全部失敗時，日線(1d)自動改抓 CoinGecko OHLC（無量能，放量條件自動關閉）
- 隱藏錯誤訊息內的 URL
- 指標：RSI(14), MACD(12/26/9), Bollinger(20,2), SMA20/50/200, ATR(14)
- 訊號：ENTER / ADD / REDUCE / EXIT / HOLD
- 文案：20/80 階梯策略提示 + EXIT 時附 20D & 3×ATR 移動停損數字
"""
...
def send_telegram(msg: str):
    """Send to Telegram if secrets present; otherwise print."""
...
def fetch_klines_binance(symbol: str, interval: str, limit: int = 300) -> pd.DataFrame:
    ...
    if last_err:
        raise last_err
    raise RuntimeError("Unknown error fetching Binance klines")
...
def fetch_ohlc_coingecko(symbol: str, limit: int = 300) -> pd.DataFrame:
    """Fallback: CoinGecko daily OHLC (no volume). Only for 1d."""
    base = symbol.upper()
    base = base.replace("USDT", "")
    cg_id = COINGECKO_ID.get(symbol.upper()) or COINGECKO_ID.get(base)
    if not cg_id:
        raise ValueError(f"No CoinGecko id mapping for {symbol}")
    # use up to 365 days (enough for SMA200)
    days = 365
    url = f"https://api.coingecko.com/api/v3/coins/{cg_id}/ohlc"
    r = requests.get(url, params={"vs_currency": "usd", "days": days}, timeout=25)
    r.raise_for_status()
    data = r.json()  # list of [timestamp, open, high, low, close]
    if not isinstance(data, list) or not data:
        raise ValueError("Empty OHLC from CoinGecko")
    df = pd.DataFrame(data, columns=["t", "open", "high", "low", "close"])
    df["close_time"] = pd.to_datetime(df["t"], unit="ms")
    df.drop(columns=["t"], inplace=True)
    df["volume"] = np.nan  # no volume info
...
def fetch_market_df(symbol: str, interval: str, limit: int = 300) -> pd.DataFrame:
    """Fetch market data with rotation + fallback."""
    try:
        return fetch_klines_binance(symbol, interval, limit)
    except Exception as e:
        # If Binance fails entirely, fallback to CoinGecko for daily only
        if interval == "1d":
            try:
                return fetch_ohlc_coingecko(symbol, limit)
            except Exception as e2:
                # raise the Binance error by default but include hint
...
def evaluate(df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
    ...
    if (price < row["sma200"]) or (macd_bear and price < row["sma50"]):
        action = "EXIT"
        reasons.append("Trend break: close < SMA200 or MACD bear & close < SMA50")
    # REDUCE
    elif (prev["close"] > prev["bb_m"] and price < row["bb_m"]) or (rsi_hot and (row["macd_hist"] < prev["macd_hist"])):
        action = "REDUCE"
        reasons.append("Momentum cooling: fell below BB mid or RSI>70 with weakening MACD")
    # ADD
        ...
        action = "ADD"
        reasons.append("Reclaim BB mid with strong RSI & rising MACD, or high-volume breakout")
    # ENTER
    elif (price > row["sma50"] and macd_bull and row["rsi14"] >= 50):
        action = "ENTER"
        reasons.append("Uptrend intact: price > SMA50, MACD bull, RSI>=50")
    ...
    for r in results_sorted:
        ...
        if reasons:
            ...
            for x in reasons:
                x = str(x)
                if " for url:" in x:
                    x = x.split(" for url:")[0].strip()
                reasons2.append(x)
            lines.append("• " + "; ".join(reasons2))
        if ref:
            if action != "ERROR":
